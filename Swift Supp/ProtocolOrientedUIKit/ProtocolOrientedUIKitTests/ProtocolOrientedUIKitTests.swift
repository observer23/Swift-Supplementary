...
final class ProtocolOrientedUIKitTests: XCTestCase {
    ...
    func testUpdateView_whenAPISuccess_showsNameEmailUsername() throws {
       let mockUser = User(id: 1, name: "Ekin", username: "ekina", email: "ekin@icloud.com", address: Address(street: "", suite: "", city: "", zipcode: "", geo: Geo(lat: "", lng: "")), phone: "", website: "", company: Company(name: "", catchPhrase: "", bs: ""))
        userService.fetchUserMockResult = .success(mockUser)
        sut.getUsers()
        XCTAssertEqual(output.updateViewArray.first?.username, "ekina")
    }
